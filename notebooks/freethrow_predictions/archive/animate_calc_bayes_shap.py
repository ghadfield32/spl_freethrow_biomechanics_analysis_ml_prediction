# all in one figure example: TOO BIG FOR STREAMLIT
...
def animate_trial_with_calc_bayes_shap_angle_meter(
    ...
    bar_plot: bool = True,
    line_plot: bool = True,
    # NEW: Optional parameter to save the animation to a file (e.g., "animation.gif" or "animation.mp4")
    save_path: Optional[str] = None,
    # NEW: If True, output as an IPython HTML object (for notebooks); otherwise return a data string for streamlit.
    ...
    NEW FEATURES:
      - notebook_mode: if True, the function returns an IPython.display.HTML object (to be used in a Jupyter Notebook)
                      rather than a raw data string. When False (the default) it behaves as before (suitable for Streamlit).
      - Deep debug logging has been added to track the output of critical functions.
      
    The function optionally saves the animation to a file if save_path is provided.
    
    Returns:
      - If notebook_mode is True: an IPython HTML object.
      - Otherwise: a string containing the inline HTML from anim.to_jshtml().
    """
    try:
        ...
        # Enable interactive 3D view controls
        (fig, ax_3d, debug=debug)
        
        # Additional subplots for polar meter, bar meter, and line graph
        ax_meter = fig.add_subplot(gs[0, 1], polar=True) if polar_plot else None
        ax_bar = fig.add_subplot(gs[1, 0]) if bar_plot else None
        ax_line = fig.add_subplot(gs[1, 1]) if line_plot else None

        # Draw court and hoop if required
        if show_court:
        ...
        # Initialize dynamic 3D elements
        ...
        # Get the filter name for the metric
        ...
        def update_func(frame: int):
            ...
            if feedback_mode.lower() == "bayesian":
                ...
                bayes_class = df.loc[frame, bayes_class_col] if bayes_class_col in df.columns else "N/A"
                bayes_unit_change_col = f"{selected_metric}_bayes_unit_change"
                bayes_unit_change = df.loc[frame, bayes_unit_change_col] if bayes_unit_change_col in df.columns else "N/A"
                ...
                shot_classification = df.loc[frame, shot_class_col] if shot_class_col in df.columns else "N/A"
                one_liner_str = (f"Ongoing {selected_metric_filter_name}: {df.loc[frame, selected_metric_filter_name]:.1f}° | "
        ...
        # (For brevity, assume that the complete code for setting up the axes, meters, and legends remains unchanged.)
        # At the end, we create the FuncAnimation:
        frames = frames_to_animate if frames_to_animate else range(len(df))
        anim = FuncAnimation(fig, update_func, frames=frames, interval=1000/30, blit=False)
        
        # If a save_path is provided and ends with ".html", save the inline HTML to that file and return it.
        if save_path is not None and save_path.endswith(".html"):
            inline_html = anim.to_jshtml()
            if debug:
                logger.debug("Generated inline HTML (for saving to file): " + repr(inline_html[:100]))
            if inline_html is None:
                logger.error("anim.to_jshtml() returned None – check animation generation or the effect of saving.")
                inline_html = "<p>Error: No animation output generated.</p>"
            with open(save_path, "w", encoding="utf-8") as f:
                f.write(inline_html)
            # In notebook mode, return the HTML object; otherwise, return the raw string.
            if notebook_mode:
            ...
            inline_html = anim.to_jshtml()
            if debug:
                logger.debug("Generated inline HTML: " + repr(inline_html[:100]))
            if inline_html is None:
                logger.error("anim.to_jshtml() returned None – check animation generation.")
                inline_html = "<p>Error: No animation output generated.</p>"
            # Return an IPython HTML object in notebook mode, else a raw data string for streamlit.
            if notebook_mode:
                return HTML(inline_html)
            else:
                return HTML(inline_html).data
    
    except Exception as e:
        logger.error(f"An unexpected error occurred in animate_trial_with_calc_bayes_shap_angle_meter: {e}")
        raise
